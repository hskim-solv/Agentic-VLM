...
- **평가셋 n=36, bootstrap CI 미보고** — ablation 간 미세 차이(예: full vs no_rerank/hierarchical)의 통계적 유의성 검증에 부족. n≥100 확장과 bootstrap CI 보고가 다음 사이클 최우선 항목. [성능표 caveat](#핵심-성능표-실측) 참고.
...
| full | agentic_full | auto | on | on | on | 0.906 | 0.929 | 0.905 | 1.000 | 0.905 | 1.000 | 0.310 | 2.7ms |
| hierarchical | agentic_full | auto | on | on | on | 0.906 | 0.929 | 0.905 | 1.000 | 0.905 | 1.000 | 0.310 | 2.8ms |
| no_metadata_first | agentic_full | auto | off | on | on | 0.844 | 0.881 | 0.679 | 0.968 | 0.857 | 1.000 | 0.000 | 3.0ms |
| no_rerank | agentic_full | auto | on | off | on | 0.906 | 0.929 | 0.905 | 1.000 | 0.905 | 1.000 | 0.310 | 2.7ms |
| no_verifier_retry | agentic_full | auto | on | on | off | 0.906 | 0.762 | 0.762 | 1.000 | 0.714 | 0.300 | 0.000 | 2.6ms |
<!-- METRICS_TABLE:END -->

> **Ablation 해석 — n=36 검출 한계 vs 실측 trade-off**: `no_rerank`와 `hierarchical`은 `full`과 primary metrics(Accuracy 0.862 / Groundedness 0.892 / Citation 0.878 / Abstention 1.000)가 동일하지만, 이는 *기능이 동등*해서가 아니라 *n=36 평가셋이 차이를 검출하지 못해서*입니다. 두 ablation 모두 실제로 다른 코드 경로를 exercise합니다 — rerank scoring weight 차이([rag_core.py:1798–1803](rag_core.py)), hierarchical reassembly([rag_core.py:1836–1838](rag_core.py)). 통계적 분리는 n≥100 평가셋에서 가능 (caveat은 line 181 참고). 한편 **명확한 설계 효용을 보이는 ablation**: `no_verifier_retry`는 accuracy +3.5pp(0.862→0.897) 대신 groundedness −16.2pp(0.892→0.730)로 trade-off가 분명하고, `no_metadata_first`는 citation precision이 0.878→0.635 (−24.3pp)로 metadata-first filtering의 효용을 직접적으로 입증합니다.
