...
본 시스템의 모든 pipeline(`naive_baseline`, `agentic_full` 포함)은 외부 LLM 호출 없이 retrieved evidence에서 claim을 추출하는 **extractive grounded-answer**입니다. Generator를 의도적으로 extractive로 한정한 4가지 이유:
...
- **평가셋 n=42, bootstrap CI 미보고** — ablation 간 미세 차이(예: full vs no_rerank/hierarchical)의 통계적 유의성 검증에 부족. n≥100 확장과 bootstrap CI 보고가 다음 사이클 최우선 항목. [성능표 caveat](#핵심-성능표-실측) 참고.
...
- **HWP native visual parsing 미지원** — 현재 코드는 `pyhwp` / `hwp5html` / `olefile` 등 native HWP parser를 import하지 않는다. HWP 본문은 `data_list.csv`의 `텍스트` 컬럼에서 **사용자가 사전 추출한 텍스트**를 사용하며 `visual_fallback_hwp` 마커가 붙는다. native binary 포맷 parsing 라이브러리 안정성/라이선스 제약 회피를 위한 의도된 trade-off이며, native parser spike는 다음 사이클 항목. [`docs/visual-ingestion-v2.md`](docs/visual-ingestion-v2.md) 참고.
...
- **External baseline 미비교** — 현재 `naive_baseline`은 저장소 내부 control. LangChain RetrievalQA, LlamaIndex SubQuestionQueryEngine 등 외부 frame과의 비교는 다음 사이클 항목.
...
|---|---:|---:|
| Overall | Answer Accuracy | 0.844 |
| Single-doc extraction | Answer Accuracy | 1.000 |
| Multi-doc comparison | Groundedness Rate | 0.700 |
| Follow-up | Answer Accuracy | 0.750 |
| Evidence | Citation Precision | 0.512 |
| Evidence | Claim Citation Alignment | 0.974 |
| Evidence | Answer Format Compliance | 0.667 |
| Abstention | Abstention Accuracy | 0.222 |
| System | Latency (p50/p95) | p50 1.4ms / p95 2.8ms |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| naive_baseline | naive_baseline | 4 | off | off | off | 0.844 | 0.714 | 0.512 | 0.974 | 0.667 | 0.300 | 0.000 | 2.8ms |
| full | agentic_full | auto | on | on | on | 0.906 | 0.929 | 0.905 | 1.000 | 0.905 | 1.000 | 0.310 | 2.9ms |
| hierarchical | agentic_full | auto | on | on | on | 0.906 | 0.929 | 0.905 | 1.000 | 0.905 | 1.000 | 0.310 | 3.2ms |
| no_metadata_first | agentic_full | auto | off | on | on | 0.844 | 0.881 | 0.679 | 0.968 | 0.857 | 1.000 | 0.000 | 2.9ms |
| no_rerank | agentic_full | auto | on | off | on | 0.906 | 0.929 | 0.905 | 1.000 | 0.905 | 1.000 | 0.310 | 3.1ms |
| no_verifier_retry | agentic_full | auto | on | on | off | 0.906 | 0.762 | 0.762 | 1.000 | 0.714 | 0.300 | 0.000 | 2.6ms |
| hybrid_bm25 | agentic_full | auto | on | on | on | 0.906 | 0.929 | 0.905 | 1.000 | 0.905 | 1.000 | 0.310 | 3.0ms |
<!-- METRICS_TABLE:END -->

> **Ablation 해석 — n=42 검출 한계 vs 실측 trade-off**: `no_rerank`와 `hierarchical`은 `full`과 primary metrics(Accuracy 0.906 / Groundedness 0.929 / Citation 0.905 / Abstention 1.000)가 동일하지만, 이는 *기능이 동등*해서가 아니라 *n=42 평가셋이 차이를 검출하지 못해서*입니다. 두 ablation 모두 실제로 다른 코드 경로를 exercise합니다 — rerank scoring weight 차이([rag_core.py:1798–1803](rag_core.py)), hierarchical reassembly([rag_core.py:1836–1838](rag_core.py)). 통계적 분리는 n≥100 평가셋에서 가능 ([성능표 caveat](#핵심-성능표-실측) 참고). 한편 **명확한 설계 효용을 보이는 ablation**: `no_verifier_retry`는 accuracy는 동일(0.906)하지만 groundedness −16.7pp(0.929→0.762), citation −14.3pp(0.905→0.762), abstention −70.0pp(1.000→0.300)로 verifier/retry 루프가 *정확도가 아니라 근거 보존·부재 판별*에 기여함을 명확히 보입니다. `no_metadata_first`는 citation precision 0.905→0.679(−22.6pp), accuracy 0.906→0.844(−6.2pp)로 metadata-first filtering의 효용을 직접적으로 입증합니다.
...
flowchart TD
    ...
    P --> RB["Lexical channel<br/>BM25 (optional, ADR 0009)"]
...
`retrieval_backend` 는 직교 축이다: 기본값 `dense` 는 기존 weighted fusion (dense + lexical + metadata) 을 그대로 사용해 `naive_baseline` (ADR 0001) 을 보존하고, `hybrid` 는 BM25 채널을 dense 와 RRF (k=60) 로 결합한다. 자세한 근거는 [ADR 0009](docs/adr/0009-hybrid-bm25-dense-retrieval-rrf.md) 참고.
...
`[A-Za-z0-9]+|[가-힣]+` 정규식과 한국어 조사 제거 normalize만 사용합니다. **의도적으로** KoNLPy / soynlp / kiwipiepy 같은 형태소 분석 라이브러리를 도입하지 않았습니다. 이유: 한자/영문 기술 용어와 한글 사업명이 혼재된 RFP 텍스트에서 형태소 분석의 정확도 이득 대비, container 부피 / CI 재현성 / Java JVM 의존(KoNLPy) 비용이 큽니다. 동일 토크나이저를 ADR 0009 의 BM25 채널에서도 그대로 재사용합니다.
