...
- **HWP native visual parsing 미지원** — CSV `텍스트` 컬럼 fallback으로 처리하며 `visual_fallback_hwp` 마커 부여. native parsing 라이브러리 부재와 라이선스 제약이 이유. [`docs/visual-ingestion-v2.md`](docs/visual-ingestion-v2.md) 참고.
...
원본 HWP를 native parse하는 대신 `data_list.csv`의 `텍스트` 컬럼을 본문 소스로 사용하고 `visual_fallback_hwp` 마커를 부여합니다. **이 경로에는 `pyhwp` / `hwp5html` / `olefile` 같은 native HWP parser가 import되지 않으며**, HWP 본문은 사용자가 사전에 `data_list.csv`로 추출해 두어야 합니다. HWP native binary 포맷의 안정적 parsing 라이브러리 부재와 라이선스 제약을 회피하기 위한 의도된 trade-off이며, native parser spike는 다음 사이클 항목입니다.
