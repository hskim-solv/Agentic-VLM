...
| [0049](./0049-kordoc-replaces-pyhwp-backend.md) | proposed | kordoc (npm subprocess, Node 18+) replaces pyhwp/hwp5 as HWP/PDF parser backend; `BIDMATE_HWP_LOADER=kordoc` (default) \| `csv_text` (offline / CI / Node-missing); `csv_text` fallback preserved (ADR 0001 invariant); table structure / headings / nested tables survive — surface paragraph-only pyhwp extraction lost; supersedes ADR 0036; closes issue #890 |
