...
| [0040](./0040-react-agent-loop-additive-preset.md) | accepted | ReAct agent loop as additive `agent_react` pipeline preset — LLM-driven Plan-Act-Observe action selection added on top of stage (B) agentic RAG; budget-capped per ADR 0041; opt-in preserves ADR 0001 / 0011 / 0020 / 0022 / 0024 invariants; closes issue #673 |
| [0041](./0041-agent-budget-cap-contract.md) | accepted | Agent budget cap contract — bounded `(steps, tokens, latency)` budget enforced by ReAct loop (ADR 0040); cost telemetry per ADR 0015; defends ADR 0003 answer dict on budget-exhausted termination; closes issue #673 |
