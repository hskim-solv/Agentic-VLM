...
| [0040](./0040-react-agent-loop-additive-preset.md) | accepted | ReAct agent loop as additive `agent_react` pipeline preset (`rag_pipeline_presets.py` + `rag_graph_react.py`); preserves ADR 0001 naive_baseline invariant, ADR 0003 answer-dict contract, and ADR 0024 3-layer default policy; closes issue #673 |
| [0041](./0041-agent-budget-cap-contract.md) | accepted | Agent budget cap contract — `react_loop` enforces two-axis hard cap (`max_iterations=5` via `BIDMATE_PLANNER_MAX_ITERATIONS`, `max_latency_ms=8000` via `BIDMATE_PLANNER_MAX_LATENCY_MS`) with `agent_budget_exceeded` abstention reason; complements ADR 0040 / ADR 0003; closes issue #673 |
