...
| 시그널 | 어디서 확인하나 |
|---|---|
| 아키텍처 결정이 **사후 합리화가 아닌 기록된 결정**으로 남아있다 | [`docs/adr/`](./adr/README.md) — 26개 ADR (18 accepted / 8 proposed), status-tracked, supersession chains 명시 |
...
| [0027](./adr/0027-lora-finetuned-embedding-additive.md) | proposed | LoRA-fine-tuned embedding adapter as additive ablation, env-var gated (`BIDMATE_EMBEDDING_LORA_ADAPTER`), HF Hub adapter pinned by commit SHA (extends 0001 / 0011 / 0019; does NOT trigger 0019 re-open) | **TODO(user)**: 시니어 관점 한 줄 직접 작성 — pretrain → fine-tune → eval cycle 시연 / HF Hub 아티팩트 supply-chain SHA pin 패턴 / ADR 0019 re-open 조건(`full` ≥+5pp)을 의도적으로 *비*트리거하는 측정 설계 등을 골라 자기 framing으로. |
